...
static guint32
convert_ascii_string_to_hex (gchar *src_buffer, guint32 src_size, guint8 *dst_buffer, guint32 dst_size)
{
	guint32 idx;
	guint32 src_index = 0;

	g_return_val_if_fail (src_buffer != NULL && src_size > 0, 0);
	g_return_val_if_fail (dst_buffer != NULL && dst_size > 0, 0);
	...
	for (idx = 0; idx < dst_size; idx++) {
		...
		dst_buffer[idx] = fu_firmware_strparse_uint8 (&src_buffer[src_index]);
	}
	return idx;
}
...
static guint32
cyacd_buffer_read_line (CyacdFileHandle* handle, gchar *line_buffer, guint32 line_buff_size)
{
	guint32 index = 0;
	guint8 ch;
...
cyacd_buffer_read_header (CyacdFileHandle *handle, guint16 *silicon_id)
{
	...
	g_autofree gchar *ascii_buffer = g_malloc0 (CYACD_ROW_ASCII_BUFFER_SIZE);
	...
	if (cyacd_buffer_read_line (handle, ascii_buffer, CYACD_ROW_ASCII_BUFFER_SIZE)) {
		if (convert_ascii_string_to_hex (ascii_buffer, sizeof(ascii_buffer), hex_buffer, sizeof(hex_buffer)) == 0) 
			return FALSE;
...
cyacd_buffer_read_row_raw_data(CyacdFileHandle *handle, guint8 *data, guint32 size)
{
	...
	if (read_size > 0) {
		if ((ascii_buffer[0] == ':') && (ascii_buffer[1] == '0') && (ascii_buffer[2] == '0')) {
			...
			if (convert_ascii_string_to_hex (&ascii_buffer[1], read_size - 1, data, size) == 0) 
				return FALSE;
...
fu_ccgx_cyacd_file_read_row (CyacdFileHandle *handle, guint8 *data, guint32 size)
{
	...
	if (read_size > 0) {
		if (ascii_buffer[0] == ':' && ascii_buffer[1] == '0' && ascii_buffer[2] == '0') { /* : and 1byte array_id */
			...
			if (convert_ascii_string_to_hex (&ascii_buffer[3], read_size - 3, data, size) == 0)
				return FALSE;
